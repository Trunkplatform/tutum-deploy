...
namespace :dockercloud do
  ...
  task :single_stack_deploy, [:service_name, :version, :ping_uri, :ping_port] do |_, args|
    ...
    begin
      @deployment = Deployment.new(dockercloud_api, service_name, version, ping_path, sleep_interval, max_timeout, dockercloud_proxy_path)
  ...
  task :dual_stack_deploy, [:service_name, :version, :router_name, :ping_uri, :ping_port] do |_, args|
    ...
    begin
      @deployment = Trunk::DockercloudApi::Deploy::Deployment
                        .new(dockercloud_api, service_name, version, ping_path, sleep_interval, max_timeout, dockercloud_proxy_path)
  ...
  task :relink_router, [:router_name, :service_uuid] do |_, args|
    ...
    begin
      @deployment = Deployment.new(dockercloud_api, nil, nil)
